...
export const fromGoJSONToGoStruct = (
  ...
  let data = JSON.parse(json.replace(/(:\s*\[?\s*-?\d*)\.0/g, "$1.1"));
  let tag = conversionParameters.tag;
...
function parseJSON(data: any, tag: string, subtag: string): string {
...
function parseNestedJSON(
  ...
  let tabDifference = Array(level + 1)
    .fill("\t")
    .join("");
